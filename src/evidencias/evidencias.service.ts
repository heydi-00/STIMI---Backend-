...
export class EvidenciasService {
  ...
  async create(createEvidenciasDto: CreateEvidenciasDto): Promise<Evidencias> {
    const evidencia = this.evidenciasRepository.create({
      ...
      ...(createEvidenciasDto.id_obligacion && {
        obligacion: { id_obligacion: createEvidenciasDto.id_obligacion },
      }),
    });
  ...
  async update(
    id: number,
    updateEvidenciasDto: UpdateEvidenciasDto,
  ): Promise<Evidencias> {
    await this.findOne(id);
    await this.evidenciasRepository.update(id, {
      ...updateEvidenciasDto,
      ...(updateEvidenciasDto.id_actividad && {
        actividad: { id_actividad: updateEvidenciasDto.id_actividad },
      }),
      ...(updateEvidenciasDto.id_obligacion && {
        obligacion: { id_obligacion: updateEvidenciasDto.id_obligacion },
      }),
    });
